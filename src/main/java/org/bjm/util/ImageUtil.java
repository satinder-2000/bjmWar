...
public class ImageUtil {
    ...
    public static BufferedImage drawEmptyPieChart(int imgSize, String text, int fontSize) {
        /*BufferedImage bimg = new BufferedImage(imgSize, imgSize, BufferedImage.TYPE_BYTE_INDEXED);
        Graphics2D g = bimg.createGraphics();
        g.setColor(Color.decode("#cc0000"));
        g.fill(new Ellipse2D.Float(0, 0, imgSize, imgSize));
        g.setColor(Color.WHITE);
        //int fontSize = 10;
        String toDraw = text.toUpperCase();
        g.setFont(new Font("sansserif", Font.PLAIN, fontSize));
        int b = g.getFontMetrics().stringWidth(toDraw);
        g.drawString(toDraw, (fontSize / 2 - b / 2), fontSize / 2 + fontSize / 3);
        g.dispose();
        return bimg;*/
